...
const listUserclient = async (req, res) => {
  ...
  const rows = await Userclient.aggregate([
    {
      $match: {
        client_id: mongoose.Types.ObjectId.createFromHexString(client_id),
      },
    },
    {
      $lookup: {
    ...
    },
    {
      $project: {
        ...
        lastName: {
          $ifNull: [{ $arrayElemAt: ['$userDetails.lastName', 0] }, 'N/A'],
        },
      },
    },
  ])
...
const getUserClient = async (req, res) => {
  const { id: assigned_id } = req.params

  const assUserClient = await Userclient.aggregate([
    {
      $match: {
        assigned_id: mongoose.Types.ObjectId.createFromHexString(assigned_id),
      },
    },
    {
      $lookup: {
    ...
    },
    {
      $project: {
        ...
        client_initials: '$clientInfo.client_initials',
      },
    },
  ])
...
export { listUserclient, addUserClient, deleteUserClient, getUserClient }
